...
def constructBayesNet(gameState: hunters.GameState):
    ...
    possibleObservations = range(0, X_RANGE + Y_RANGE + (MAX_NOISE - 1))
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            # print("position =", pos)
            # print("position belief =", self.getBeliefDistribution()[pos])

            beliefs[pos] *= self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition())

        # error case if total is zero
        if beliefs.total() == 0:
            # from spec
            self.initializeUniformly(gameState)
            return
        ...
        for i in range(self.numParticles):
            tempParticles.append(beliefs.sample())
